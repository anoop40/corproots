...
class DocumentList extends StatefulWidget{
  _DocumentList createState() => _DocumentList();
}
class _DocumentList extends State<DocumentList>{
  ...
  Future MyDocuments() async {
    ...
    var response = await http.post(Uri.parse(serverUrlfinal), headers: {
      ...
      "userId": prefs.getString('fetchingUsersId'),
    });
    //print("From server documents : " + response.body);
    this.setState(() {
      myDocs = json.decode(response.body) as List<dynamic>;
    });
    if (prefs.containsKey("customMessageToClientToUploadDoc")) {
      this.setState(() {
        _customeMessage.text =
            prefs.getString("customMessageToClientToUploadDoc")!;
      });
    }
                          ...
                          children: <Widget>[
                            Container(
                              ...
                              child: ListTile(
                                ...
                                title: Text(StringUtils.capitalize(dataFinal['file_heading'])),
                                onTap: () async {
                                  ...
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) => WebViewNewLoad(
                                            dataFinal['file_name'],
                                            dataFinal['id'])),
                                  );
                                },
